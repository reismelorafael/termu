...
def test_truth_table_keeps_real_package_stack_unproved():
    ...
    for token in ['`pkg` | TOKEN_VAZIO', '`apt` | TOKEN_VAZIO', '`apt-get` | TOKEN_VAZIO', '`dpkg` | TOKEN_VAZIO', '`libapt` | TOKEN_VAZIO', '`proot` | TOKEN_VAZIO', 'certificados | TOKEN_VAZIO', 'DNS/network básico | TOKEN_VAZIO', 'repositório configurado | TOKEN_VAZIO']:
        assert token in text
    package_rows = [line for line in text.splitlines() if any(key in line for key in ('`pkg`', '`apt`', '`apt-get`', '`dpkg`', '`libapt`', '`proot`', 'certificados', 'DNS/network básico', 'repositório configurado'))]
